...
| 2026-05-28 | **PROMPT DEFINITIVO — Migração para ESM (.mjs) na Vercel**: Resolvido o erro `SyntaxError: Cannot use import statement outside a module` migrando o formato do bundle de CJS para ESM nativo. Script `"build"` alterado para `--format=esm --outfile=api/index.mjs`. `vercel.json` atualizado com entrypoint `api/index.mjs` e `includeFiles` para `@react-pdf/**`. Deletado `api/package.json` (não mais necessário com extensão `.mjs`). Refatorado `generatePdfStream` com fallback robusto para `renderToStream` e logs de diagnóstico. Corrigido import do `firebase-admin` com `ServiceAccount` e proteção `!admin.apps.length` contra inicialização duplicada. Build local validado (`Exit code: 0`, `api/index.mjs` 26.8mb). | `api/index.tsx`, `package.json`, `vercel.json`, `api/package.json` (deletado) |
