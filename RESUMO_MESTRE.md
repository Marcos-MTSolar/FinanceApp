...
| 2026-05-28 | **PROMPT DEFINITIVO — Resolução de Conflito de Rotas e Configuração de Gitignore**: Corrigido o erro de deploy imediato `Two or more files have conflicting paths or names. The path 'api/index.js' has conflicts with 'api/index.tsx'`. Removido o arquivo `api/index.js` compilado do controle de versão (`git rm --cached`). Adicionados `api/index.js` e `api/index.mjs` ao `.gitignore` para prevenir conflitos futuros no deploy serverless da Vercel. | `.gitignore`, `api/index.js` (removido do Git) |
